...
export default function OrganismePage() {
 ...
 const downloadRgpdPdf = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(
        ...
        XLSX.utils.json_to_sheet([
          { generated_at: d.generated_at ?? null, user_id: d.user_id ?? null, email: d.email ?? null },
        ]),
        "meta"
      );
  ...
  const deleteAccount = async () => {
    ...
    try {
      setDeleting(true);
      alert("STEP 1: appel RPC delete_my_account...");

      const { error } = await supabase.rpc("delete_my_account");

      if (error) {
        alert("STEP 2: RPC ERROR -> " + error.message);
        setDeleting(false);
        setMessage(error.message);
        return;
      }

      alert("STEP 3: RPC OK -> signOut");
      await supabase.auth.signOut();

      alert("STEP 4: redirect /");
      window.location.href = "/";
    } catch (e: any) {
      setDeleting(false);
      alert("STEP X: EXCEPTION JS -> " + (e?.message ?? String(e)));
      setMessage(e?.message ?? String(e));
    }
  };
      ...
      {isInvitedOnly ? (
        ...
        <>
          ...
          <div className="card space-y-4">
            ...
            {exportConfirmTouched && !exportConfirmed && (
              <div className="text-sm text-red-700">Coche cette case après avoir exporté.</div>
            )}
          </div>
